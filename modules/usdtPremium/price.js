...
/**
 * GET /api/usdt-premium/price/:months — Fragment premium slot (usdt_premium_N).
 */
export async function getUsdtPremiumPrice(req, res, ctx) {
  const {
    pool,
    PREMIUM_3,
    PREMIUM_6,
  ...
  const slotKey = usdtPremiumSlotKey(months);
  ...
  try {
    ...
    console.error("⚠️ USDT premium conflict prices:", err.message);
  }
